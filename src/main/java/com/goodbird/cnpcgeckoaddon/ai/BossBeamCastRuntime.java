...
final class BossBeamCastRuntime {
    ...
    boolean tryStart(ServerLevel level, TeleportPathData data, BossPhaseData phase, long gameTime) {
        if (!phase.isBeamEnabled() || gameTime < boss.abilityScheduleAt(BossAbility.BEAM)) return false;
        ...
        if (BossBeamScheduler.isSweeping(npc)) {
            boss.setAbilityScheduleAt(BossAbility.BEAM, gameTime + RETRY_TICKS);
            return false;
        }
        ...
        boss.commitYaw(phase.getBeamStartMode() == BossPhaseData.BEAM_START_RANDOM
                ? npc.getRandom().nextFloat() * FULL_TURN_DEGREES : npc.getYRot());
        boss.beginAction(BossAbility.BEAM, phase.getBeamAnimation(),
        ...
        boss.setAbilityScheduleAt(BossAbility.BEAM, gameTime + phase.getBeamActionDelayTicks()
                + boss.rageDown(phase.getBeamCooldownTicks()));
        return true;
    }

    boolean hasTargets(ServerLevel level, BossPhaseData phase) {
        return !boss.beamVictims(level, BossBeamScheduler.centreOf(npc), phase.getBeamLength()).isEmpty();
    }
    ...
    void perform(ServerLevel level, BossPhaseData phase, long gameTime) {
        BossBeamScheduler.start(level, npc, phase, boss.committedYaw(),
                boss.rageUp(phase.getBeamDamage()), boss.rageUp(phase.getBeamKnockback()), gameTime);
    }
